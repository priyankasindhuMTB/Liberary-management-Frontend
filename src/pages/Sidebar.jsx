...
const adminNavItems = [
  ...
  { path: "/create-shifts", label: "Shifts",   icon: Clock      },
  // { path: "/create-admin-direct", label: "Create Admin", icon: UserPlus }, // 👈 Added here
];
...
const Sidebar = () => {
  ...
  useEffect(() => {
    try {
      ...
      // 👇 AUTO-REDIRECT GUARD: Protects view isolation boundaries
      if (data?.role === "super_admin") {
        const structuralPaths = ["/super-admin", "/all-admins"];
        if (!structuralPaths.includes(location.pathname)) {
          navigate("/all-admins"); // Bounce root profile safely out of regular library workspaces
        }
      }
    } catch { 
      setAdmin(null); 
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("token");
    localStorage.removeItem("admin");
    navigate(isSuper ? "/super-admin/login" : "/login");
  };
